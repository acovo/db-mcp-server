...
- `get_info()` embeds connection info into server instructions (avoids extra `list_connections` call)
...
**Tools (`src/tools/`)**
- Each tool in separate module: query, execute, schema, transaction, explain, connection
...
- `connection.rs`: Runtime connection management (add, update, delete)
...
**JSON Schema Compatibility (Critical)**
- All structs deriving `schemars::JsonSchema` MUST have `#[schemars(transform = schemars::transform::RestrictFormats::default())]`
- This strips `format` fields (e.g., `"format": "uint32"`) from generated JSON Schema
- Without this, some MCP clients (particularly stdio-based) fail with "Invalid server type" during tool listing
- Applies to ALL input/output structs used in MCP tool definitions

## Code Style

**Rustfmt**: Edition 2024, max width 100, tab spaces 4, Unix newlines.

**Clippy**: MSRV 1.85.0, cognitive complexity threshold 25, too many arguments threshold 7.

**Comments**: Public API items (`pub`) require `///` documentation. Private items should have NO comments unless they involve subtle invariants, safety requirements, performance trade-offs, or bug workarounds. Prefer refactoring unclear code into well-named functions over adding comments. Delete outdated comments immediately. Never commit commented-out code.

**Simplicity**: YAGNI. Write only what is needed now. No unused features, unnecessary compatibility layers, premature abstractions, or defensive programming for internal code. Public API boundaries are the exception -- do add proper validation and error handling there.
...
3. Add `#[schemars(transform = schemars::transform::RestrictFormats::default())]` to ALL JsonSchema structs
4. Implement tool handler logic
5. Add tool to `src/mcp/service.rs` using `#[tool]` macro
